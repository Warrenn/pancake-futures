...
async function executeTrade({
    ...
    if ((logCount % logFrequency) == 0) {
        log(`price:${price} sp:${strikePrice} q:${size} p(${putOption?.symbol}):${putOption?.unrealisedPnl}`);
        logCount = 1;
    }
    else logCount++;

    if (!putSymbol || strikePrice == 0) ({ putSymbol, strikePrice, expiry: expiry } = getPutSymbol(price));
    if (!putOption || price > strikePrice || price == 0) return;

    await buyPutOrder(size, putSymbol);
}
